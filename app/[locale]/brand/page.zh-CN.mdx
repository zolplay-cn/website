...
<aside className='not-prose mt-6 rounded-md border border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900/50 p-4 text-[13px] leading-6'>
  <b>友情提醒：</b> Zolplay 的图形素材为专有资产，受知识产权法律保护。未经 Zolplay
  书面许可，请勿修改文件、暗示任何背书，或与其他标识组合使用。若有疑问，请发送邮件至{' '}
...
当需要横向的品牌呈现时，请优先使用提供好的 Zolplay 组合标识（千纸鹤图形标 + 文字标）。不要自行调整两者间距或对齐关系，只在批准的浅色或深色配色中使用。
...
在紧凑布局、头像或仅图标网格的场景，可使用 Zolplay 千纸鹤图形标。请谨慎使用: 只要空间允许，优先使用组合标识或文字标识。

<Grid cols={1} md={2} gap={4} className='my-5'>
  <GridItem
    className='bg-neutral-50'
    action={{ kind: 'download', href: '/assets/brand/logo-dark.svg', label: '下载 SVG' }}
  >
    <div className='w-full px-5 py-4 md:px-7 md:py-5 flex items-center justify-center'>
  ...
  <GridItem
    ...
    action={{ kind: 'download', href: '/assets/brand/logo-light.svg', label: '下载 SVG' }}
  >
    <div className='w-full px-5 py-4 md:px-7 md:py-5 flex items-center justify-center'>
