...
export function settingsStructure(typeDef: DocumentDefinition): StructureResolver {
  return (S) => {
    ...
    const defaultListItems = S.documentTypeListItems().filter((listItem) => listItem.getId() !== typeDef.name)
